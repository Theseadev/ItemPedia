...
<div id="floatingChatDockContainer" class="select-none">
    ...
    <div id="floatingChatWindow" 
         class="hidden fixed bottom-4 right-4 sm:right-6 z-50 w-[95vw] sm:w-[760px] md:w-[820px] h-[540px] sm:h-[580px] bg-white border border-slate-300/80 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div class="px-4 py-3 bg-white border-b border-slate-200 flex items-center justify-between flex-shrink-0">
            <div class="flex items-center gap-2">
                <i class="fa-solid fa-envelope text-slate-700 text-sm"></i>
                <span class="font-extrabold text-slate-900 text-sm">Pesan</span>
            </div>

            <!-- Minimize / Collapse Button (Down Chevron) -->
            <button type="button" 
                    onclick="toggleFloatingChat(false)" 
                    class="w-7 h-7 rounded-lg text-slate-500 hover:text-slate-900 hover:bg-slate-100 flex items-center justify-center transition" 
        ...
        <div class="flex-grow flex overflow-hidden">
            ...
            <div class="w-64 sm:w-72 border-r border-slate-200 flex flex-col bg-white flex-shrink-0">
                
                <!-- Filter Dropdown & History Link -->
                <div class="p-3 border-b border-slate-100 space-y-2">
                    <div class="relative">
                        <select id="chatInboxFilter" onchange="filterChatInbox(this.value)" class="w-full px-3 py-1.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-700 focus:outline-none focus:border-blue-500 shadow-2xs cursor-pointer">
                    ...
                    <div class="flex items-center gap-1.5 text-[11px] text-slate-400 px-1 font-medium">
                        ...
                        <span>Lihat riwayat pesan <a href="/admin/orders" class="text-blue-600 font-bold hover:underline">di sini</a></span>
                    </div>
                </div>

                <!-- Conversation Items List Container -->
                <div id="chatConversationList" class="flex-grow overflow-y-auto divide-y divide-slate-50 no-scrollbar">
            ...
            <div id="chatRightPanel" class="flex-grow flex flex-col bg-white overflow-hidden relative">
                
                <!-- Active Conversation Header -->
                <div id="chatActiveHeader" class="p-3 px-4 border-b border-slate-200 flex items-center justify-between bg-white flex-shrink-0">
                    <div class="flex items-center gap-2.5 min-w-0">
                        ...
                        <div class="min-w-0">
                            <div class="flex items-center gap-1.5">
                                <span id="chatActiveUsername" class="font-extrabold text-slate-900 text-sm truncate">Jajie</span>
                                <span id="chatActiveCountry" class="text-xs font-bold text-slate-400">MY</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center gap-2">
                        <a id="chatActiveOrderBtn" 
                           href="#" 
                           target="_blank" 
                           class="px-3 py-1.5 rounded-xl border border-slate-200 hover:bg-slate-50 text-slate-700 text-xs font-bold flex items-center gap-1.5 shadow-2xs transition active:scale-95">
                            <span>Daftar Pesanan</span>
                            <i class="fa-solid fa-bag-shopping text-slate-400 text-xs"></i>
                        </a>
                    </div>
                </div>

                <!-- Message Bubbles Scroll Area -->
                <div id="chatMessageStream" class="flex-grow p-4 space-y-3 overflow-y-auto bg-white text-xs no-scrollbar">
                    <!-- Messages will be populated here via JavaScript -->
                </div>

                <!-- Quick Suggestions / VIP Server Link Chip Strip -->
                <div class="px-3 py-1.5 bg-slate-50/70 border-t border-slate-100 flex items-center gap-1.5 overflow-x-auto no-scrollbar text-[11px]">
                    <button type="button" onclick="sendQuickChatPreset('https://www.roblox.com/games/share?code=' + Math.random().toString(36).substring(2,10) + '&type=Server')" class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-blue-600 font-bold hover:bg-blue-50 whitespace-nowrap transition shadow-2xs">
                        🔗 Kirim Link Private Server
                    </button>
                    <button type="button" onclick="sendQuickChatPreset('Ok siap kak, item sedang disiapkan ya!')" class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-slate-700 font-semibold hover:bg-slate-100 whitespace-nowrap transition shadow-2xs">
                        Item disiapkan
                    </button>
                    <button type="button" onclick="sendQuickChatPreset('ntar mimin ganti akun dulu ya kak')" class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-slate-700 font-semibold hover:bg-slate-100 whitespace-nowrap transition shadow-2xs">
                        Ganti akun dulu
                    </button>
                    <button type="button" onclick="sendQuickChatPreset('Makasih banyak ya kak sudah order!')" class="px-2.5 py-1 rounded-lg bg-white border border-slate-200 text-slate-700 font-semibold hover:bg-slate-100 whitespace-nowrap transition shadow-2xs">
                        Terima kasih
                    </button>
                </div>
                    ...
                    <form id="floatingChatForm" onsubmit="handleSendFloatingChat(event)" class="flex items-center gap-2">
                        ...
                        <div class="relative flex-grow">
                            <input type="text" 
                                   ...
                                   autocomplete="off"
                                   class="w-full pl-3.5 pr-9 py-2 bg-white border border-slate-200 rounded-xl text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:border-blue-500 shadow-2xs">
                        ...
                        <button type="submit" 
                                id="floatingBtnSend" 
                                class="w-9 h-9 rounded-full bg-slate-800 hover:bg-slate-900 text-white flex items-center justify-center text-xs shadow-xs transition active:scale-95 flex-shrink-0" 
                                title="Kirim Pesan">
...
function renderInboxList(list) {
    ...
    if (list.length === 0) {
        container.innerHTML = '<div class="p-6 text-center text-slate-400 text-xs">Tidak ada percakapan.</div>';
        return;
    }
    ...
    container.innerHTML = list.map(c => {
        ...
        return `
            <div onclick="selectChatConversation('${c.invoice_number}')" 
                 class="p-3 sm:p-3.5 flex items-start gap-2.5 cursor-pointer transition ${isSelected ? 'bg-blue-50/80 border-l-4 border-l-blue-600' : 'hover:bg-slate-50'}">
                ...
                <div class="min-w-0 flex-grow">
                    <div class="flex items-center justify-between gap-1">
                        <span class="font-extrabold text-slate-900 text-xs sm:text-sm truncate">${escapeChatHtml(c.roblox_username)}</span>
                        <span class="text-[10px] text-slate-400 font-semibold flex-shrink-0">${c.time_formatted || ''}</span>
                    </div>

                    <div class="flex items-center justify-between gap-1 mt-0.5">
                        <p class="text-xs text-slate-500 font-normal truncate max-w-[130px] sm:max-w-[150px]">
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.order) {
                ...
                const cardHtml = `
                    <div class="mx-auto max-w-sm p-3.5 rounded-2xl border border-slate-200 bg-white shadow-xs my-2">
                        <div class="flex items-start gap-3">
                            <div class="w-12 h-12 rounded-xl bg-amber-100/70 border border-amber-200 flex items-center justify-center text-amber-500 text-xl flex-shrink-0 p-1 overflow-hidden">
                                <img src="${ord.image_url}" alt="" class="w-full h-full object-cover rounded-lg" onerror="this.src='/images/logo-icon.png'">
                            </div>
                            <div class="min-w-0 flex-grow">
                                <span class="px-2 py-0.5 rounded-md text-[10px] font-bold bg-amber-100/80 text-amber-800 inline-block mb-0.5">
                                    ${statusLabel}
                                </span>
                                <h4 class="text-xs font-black text-slate-900 uppercase tracking-tight truncate leading-tight" title="${escapeChatHtml(ord.product_name)}">
                                    ${escapeChatHtml(ord.product_name)}
                                </h4>
                                <div class="text-xs font-black text-amber-600 mt-0.5">
                        ...
                        <a href="/order/${encodeURIComponent(ord.invoice_number)}" 
                           target="_blank" 
                           class="mt-2.5 w-full py-2 rounded-xl border border-blue-600 text-blue-600 hover:bg-blue-50 text-xs font-bold block text-center transition">
                            Lihat Pesanan
                        </a>
                    </div>
                `;
...
function formatChatMessageHtml(rawText, isSeller) {
    ...
    let formatted = escaped.replace(urlPattern, function(url) {
        ...
        return `<a href="${url}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-800 underline font-bold break-all inline-flex items-center gap-1 bg-white/70 hover:bg-white px-1.5 py-0.5 rounded transition"><span>${url}</span><i class="fa-solid fa-arrow-up-right-from-square text-[9px] text-blue-500"></i></a>`;
    });
    ...
    if (!detectedRobloxUrl) {
        ...
        formatted = formatted.replace(rawCodePattern, function(match) {
            ...
            return `<a href="${fullRobloxUrl}" target="_blank" rel="noopener noreferrer" class="text-blue-600 hover:text-blue-800 underline font-bold break-all inline-flex items-center gap-1 bg-white/70 hover:bg-white px-1.5 py-0.5 rounded transition"><span>${match}</span><i class="fa-solid fa-arrow-up-right-from-square text-[9px] text-blue-500"></i></a>`;
        });
    }
...
function appendSingleMessageBubble(msg) {
    ...
    if (isSeller) {
        // Outgoing Bubble (Light Sky Blue bg-blue-100 / bg-[#e0f2fe])
        bubbleHtml = `
            <div class="flex justify-end items-end gap-1.5 my-1.5">
                <div class="space-y-0.5 max-w-[85%] text-right">
                    <div class="p-3 px-3.5 rounded-2xl rounded-tr-xs bg-[#e0f2fe] text-slate-800 text-xs font-medium text-left leading-relaxed break-words shadow-2xs">
                        ${formatChatMessageHtml(msg.message, true)}
                    </div>
                    <div class="text-[10px] text-slate-400 font-semibold flex items-center justify-end gap-1 px-1">
                        <span>${msg.time_formatted || ''}</span>
                        <i class="fa-solid fa-check-double text-blue-500 text-[9px]" title="Terkirim"></i>
                    </div>
                </div>
            </div>
        `;
    } else {
        // Incoming Bubble (White with border)
        bubbleHtml = `
            <div class="flex justify-start items-start gap-1.5 my-1.5">
                <div class="space-y-0.5 max-w-[85%]">
                    <div class="p-3 px-3.5 rounded-2xl rounded-tl-xs bg-white border border-slate-200 text-slate-800 text-xs font-medium leading-relaxed break-words shadow-2xs">
