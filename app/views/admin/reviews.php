...
<body class="bg-[#f0f2f5] text-slate-800 min-h-screen flex flex-col antialiased selection:bg-blue-100 selection:text-blue-700">
    ...
    <main id="adminMainArea" class="md:pl-72 flex-grow transition-all duration-300 flex flex-col">
        ...
        <div class="max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6 flex-grow">
            ...
            <!-- Page Title -->
            <div>
                <h1 class="text-xl sm:text-2xl font-black text-slate-900 tracking-tight">Ulasan Pembeli</h1>
                <p class="text-xs text-slate-400 mt-0.5">Yuk, cek ulasan dari pembeli untuk meningkatkan performa tokomu.</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                ...
                <div class="lg:col-span-4 bg-white rounded-2xl border border-slate-200/90 shadow-xs p-5 sm:p-6 space-y-5">
                    <div class="flex items-center justify-between">
                        <h2 class="text-sm font-extrabold text-slate-900 uppercase tracking-wider">Rating Toko</h2>
                        <span class="text-[11px] font-bold text-slate-400"><?= number_format($totalReviews, 0, ',', '.') ?> Ulasan</span>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <div class="flex items-baseline gap-1">
                            <span class="text-3xl sm:text-4xl font-black text-slate-900">5.0</span>
                    ...
                    <div class="space-y-2.5 pt-2 border-t border-slate-100">
                        ...
                        <div class="flex items-center gap-2.5 text-xs font-semibold text-slate-600">
                            ...
                            <span class="w-14 text-right text-[11px] font-bold text-slate-500">
                                <?= number_format($cnt, 0, ',', '.') ?>
                            </span>
                        </div>
                        <?php endfor; ?>
                ...
                <div class="lg:col-span-8 space-y-4">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-4 sm:p-5 space-y-3.5">
                        ...
                        <form method="GET" action="/admin/reviews" class="flex flex-col sm:flex-row items-center gap-3">
                            ...
                            <div class="w-full sm:w-48">
                                <select class="w-full px-3.5 py-2.5 bg-white border border-slate-200 rounded-xl text-xs font-semibold text-slate-700 focus:outline-none focus:border-blue-500 shadow-2xs">
                        ...
                        <div class="flex items-center gap-2 overflow-x-auto no-scrollbar text-xs pt-1 border-t border-slate-100">
                            <a href="/admin/reviews?category=<?= urlencode($filterCategory) ?>" 
                               class="px-4 py-1.5 rounded-full font-bold transition whitespace-nowrap <?= ($filterStar === 0) ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50' ?>">
                                Semua
                            </a>
                            <?php for ($s = 5; $s >= 1; $s--): ?>
                            <a href="/admin/reviews?star=<?= $s ?>&category=<?= urlencode($filterCategory) ?>" 
                               class="px-3.5 py-1.5 rounded-full font-bold transition whitespace-nowrap flex items-center gap-1.5 <?= ($filterStar === $s) ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-white text-slate-600 border border-slate-200 hover:bg-slate-50' ?>">
                    ...
                    <div class="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-12 text-center">
                        <div class="w-20 h-20 mx-auto rounded-full bg-amber-50 text-amber-400 flex items-center justify-center text-3xl mb-3">
                            <i class="fa-regular fa-star"></i>
                        </div>
                        <h3 class="font-bold text-slate-800 text-sm">Belum ada ulasan pembeli</h3>
                        <p class="text-xs text-slate-400 mt-1">Ulasan dari pembeli akan muncul otomatis di sini setelah pesanan selesai.</p>
                    </div>
                    <?php else: ?>
                    <div class="space-y-3">
                        <?php foreach ($reviews as $r): ?>
                        <div class="bg-white rounded-2xl border border-slate-200/90 shadow-xs p-4 sm:p-5 space-y-3 hover:border-slate-300 transition">
                            ...
                            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                                <div class="flex items-center gap-3">
                                    <img src="<?= htmlspecialchars($r['roblox_avatar_url'] ?: 'https://ui-avatars.com/api/?name=' . urlencode($r['roblox_username']) . '&background=2563eb&color=fff') ?>" 
                                         alt="" 
                                         class="w-10 h-10 rounded-full border border-slate-200 bg-slate-50 object-cover flex-shrink-0 shadow-2xs">
                                    <div>
                                        <div class="flex items-center gap-2">
                                            <span class="font-extrabold text-slate-900 text-xs sm:text-sm">
                                                <?= htmlspecialchars($r['roblox_username']) ?>
                                            </span>
                                            <span class="inline-flex items-center gap-1 text-[10px] font-black bg-blue-50 text-blue-700 px-2 py-0.5 rounded-md border border-blue-100">
                                        ...
                                        <div class="flex items-center gap-1 text-amber-400 text-xs mt-0.5">
                                            ...
                                            <?php endfor; ?>
                                            <span class="text-slate-400 text-[11px] ml-1 font-bold"><?= number_format($r['rating'], 1) ?></span>
                                        </div>
                                    </div>
                                </div>

                                <span class="text-[11px] font-semibold text-slate-400 self-start sm:self-auto">
                            ...
                            <div>
                                <span class="inline-block text-[11px] font-semibold text-slate-600 bg-slate-100 px-2.5 py-1 rounded-lg">
                                    Produk: <strong class="text-slate-900"><?= htmlspecialchars($r['product_name'] ?? 'Item Game') ?></strong>
                                </span>
                            </div>

                            <!-- Comment Content -->
                            <div class="text-xs text-slate-700 leading-relaxed bg-slate-50/60 p-3 rounded-xl border border-slate-100 font-medium">
